...
| m80 | The full suite, every operation, every lifecycle path, every error, the clock hook making time-dependent cases instant |
...
Their UAT suite is 63 Robot Framework cases driven through kubectl against a live EKS cluster. Pointing the operator's SDK at m80 and running that suite on k3d is the external conformance prize. It tests through a real consumer's eyes, covers drift and auto-suspend behavior m80's levers exist for, and any pass rate becomes the headline number in the conversation with codriverlabs. This layer is theirs, not ours, which is what makes it credible.
...
4. Only then, implement m80 against the suite.
